...
ntuples = str(year)
signal = ["output_"+str(sig)+"_XToYHTo2b2g_MX-"+str(mass_range)+"mass_"+str(Mjjbin)+"-Mjj.root"]
diphotonJets = ["output_DiPhotonJetsBox_MGG-80toInf_13TeV-Sherpa_"+str(Mjjbin)+"-Mjj.root"]
#2016
gJets_lowPt = ["output_GJet_Pt-20to40_DoubleEMEnriched_MGG-80toInf_Tune"+str(tune)+"_13TeV_Pythia8_"+str(Mjjbin)+"-Mjj.root"]
gJets_highPt = ["output_GJet_Pt-40toInf_DoubleEMEnriched_MGG-80toInf_Tune"+str(tune)+"_13TeV_Pythia8_"+str(Mjjbin)+"-Mjj.root"]

ggH = ["output_GluGluHToGG_M-125_13TeV_powheg_pythia8_"+str(Mjjbin)+"-Mjj.root"]
vbf = ["output_VBFHToGG_M-125_13TeV_powheg_pythia8_"+str(Mjjbin)+"-Mjj.root"]
VH = ["output_VHToGG_M125_13TeV_amcatnloFXFX_madspin_pythia8_"+str(Mjjbin)+"-Mjj.root"]
bbH = ["output_bbHToGG_M-125_4FS_yb2_13TeV_amcatnlo_"+str(Mjjbin)+"-Mjj.root"]
ttH = ["output_ttHToGG_M125_13TeV_powheg_pythia8_"+str(Mjjbin)+"-Mjj.root"]
Data= ["Data_"+str(Mjjbin)+"-Mjj.root"]
...
preprocessing.set_signals_and_backgrounds("bbggtrees_13TeV_DoubleHTag_0",branch_names+extra_branches,shuffle=False)
X_bkg,y_bkg,weights_bkg,X_sig,y_sig,weights_sig=preprocessing.set_variables(branch_names)

X_data,y_data,weights_data = preprocessing.set_data("Data_13TeV_DoubleHTag_0",branch_names)
...
###########
##2016, low-X_low-Mjj
loaded_model = joblib.load(os.path.expanduser(str(pklfolder)+'/'+mass_range+'-X_'+str(Mjjbin)+'-Mjj_XGB_training_file.pkl'))
...
outTag = 'flattrees_NMSSM_fromjobs/hadd_files/training_trees/flattening_X-'+str(mass_range)+'mass_'+str(Mjjbin)+"-Mjj_"+str(year)
...
#Save Signal
sig_count_df = rpd.read_root(utils.IO.signalName[0],"bbggtrees_13TeV_DoubleHTag_0", columns = branch_names+additionalCut_names)
...
bkg_1_count_df = rpd.read_root(utils.IO.backgroundName[1],"bbggtrees_13TeV_DoubleHTag_0", columns = branch_names+additionalCut_names)
...
bkg_2_count_df = rpd.read_root(utils.IO.backgroundName[2],"bbggtrees_13TeV_DoubleHTag_0", columns = branch_names+additionalCut_names)
...
for iProcess in range(0,len(utils.IO.backgroundName)):
    ...
    bkg_count_df = rpd.read_root(utils.IO.backgroundName[iProcess],"bbggtrees_13TeV_DoubleHTag_0", columns = branch_names+additionalCut_names)
...
#save Data                                                                                                                                                                                                                                                     
data_count_df = rpd.read_root(utils.IO.dataName[0],"Data_13TeV_DoubleHTag_0", columns = branch_names+additionalCut_names)
